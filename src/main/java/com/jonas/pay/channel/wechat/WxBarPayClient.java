...
public class WxBarPayClient extends AbstractWxPayClient {
    ...
    protected PayOrderRespDTO doUnifiedOrderV2(PayOrderUnifiedReqDTO reqDTO) throws WxPayException {
        ...
        LocalDateTime expireTime = LocalDateTimeUtil.addTime(AUTH_CODE_EXPIRE);
        ...
        for (int i = 1; i < Byte.MAX_VALUE; i++) {
            try {
                ...
                PayOrderRespDTO payOrderRespDTO = PayOrderRespDTO.successOf(response.getTransactionId(),
                        response.getOpenid(), parseDateV2(response.getTimeEnd()), response.getOutTradeNo(), response);
                ...
                log.info("[doUnifiedOrderV2][发起微信 Bar 支付第({})失败，等待下一轮重试，请求({})，响应({})]", i,
                        GsonUtil.toJson(request), ex.getMessage());
    ...
    static String getAuthCode(PayOrderUnifiedReqDTO reqDTO) {
        ...
        if (StrUtil.isEmpty(authCode)) {
            throw new BizException(ErrorCode.WECHAT_PAY_ERROR1);
        }
        return authCode;
    }
