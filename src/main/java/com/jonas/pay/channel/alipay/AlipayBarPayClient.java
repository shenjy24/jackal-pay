...
public class AlipayBarPayClient extends AbstractAlipayPayClient {
    ...
    public PayOrderRespDTO doUnifiedOrder(PayOrderUnifiedReqDTO reqDTO) throws AlipayApiException {
        ...
        if (StrUtil.isEmpty(authCode)) {
            log.error("条形码不能为空");
            throw new BizException(ErrorCode.ALIPAY_ERROR3);
        }
        ...
        if ("10000".equals(response.getCode())) { // 免密支付
            ...
            PayOrderRespDTO payOrderRespDTO = PayOrderRespDTO.successOf(response.getTradeNo(),
                    response.getBuyerUserId(), successTime, response.getOutTradeNo(), response);
