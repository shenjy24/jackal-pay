package com.jonas.pay.channel.alipay;
...
import static com.jonas.pay.channel.alipay.AlipayPayClientConfig.MODE_CERTIFICATE;
...
public abstract class AbstractAlipayPayClient extends AbstractPayClient<AlipayPayClientConfig> {
    ...
    protected PayRefundRespDTO doUnifiedRefund(PayRefundUnifiedReqDTO reqDTO) throws AlipayApiException {
        ...
        if (!response.isSuccess()) {
            ...
            if (ObjectUtil.equalsAny(response.getSubCode(), "ACQ.SYSTEM_ERROR", "SYSTEM_ERROR")) {
    ...
    protected PayRefundRespDTO doGetRefund(String outTradeNo, String outRefundNo) throws AlipayApiException {
        ...
        if (!response.isSuccess()) {
            ...
            if (ObjectUtil.equalsAny(response.getSubCode(), "TRADE_NOT_EXIST", "ACQ.TRADE_NOT_EXIST")) {
    ...
    protected PayTransferRespDTO doUnifiedTransfer(PayTransferUnifiedReqDTO reqDTO) throws AlipayApiException {
        ...
        if (!Objects.equals(config.getMode(), MODE_CERTIFICATE)) {
            throw new BizException(ErrorCode.ALIPAY_ERROR1);
        }
        ...
        if (reqDTO.getChannelExtras() != null) {
            model.setBusinessParams(GsonUtil.toJson(reqDTO.getChannelExtras()));
        }
        // ② 个性化的参数
        ...
        switch (transferType) {
            ...
            default: {
                log.error("不正确的转账类型: {}", transferType);
                throw new BizException(ErrorCode.ALIPAY_ERROR2);
            }
        }
        ...
        if (!response.isSuccess()) {
            ...
            if (ObjectUtil.equalsAny(response.getSubCode(), "PAYMENT_INFO_INCONSISTENCY", "SYSTEM_ERROR", "ACQ.SYSTEM_ERROR")) {
            ...
            if (ObjectUtil.equalsAny(response.getStatus(), "REFUND", "FAIL")) { // 转账到银行卡会出现 "REFUND" "FAIL"
                return PayTransferRespDTO.closedOf(response.getSubCode(), response.getSubMsg(),
                        reqDTO.getOutTransferNo(), response);
            }
    ...
    protected PayTransferRespDTO doGetTransfer(String outTradeNo, PayTransferTypeEnum type) throws Throwable {
        ...
        if (response.isSuccess()) {
            ...
            if (ObjectUtil.equalsAny(response.getSubCode(), "ORDER_NOT_EXIST", "SYSTEM_ERROR", "ACQ.SYSTEM_ERROR")) {
